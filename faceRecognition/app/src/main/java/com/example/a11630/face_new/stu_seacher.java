...
public class stu_seacher extends AppCompatActivity implements View.OnClickListener {

    Button btn_choosetime, btn_confirm;
    TextView search_sum;
    int flag;
    EditText IDS;
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        search_sum = (TextView) findViewById(R.id.stu_tv_sum);
        btn_choosetime = (Button) findViewById(R.id.stu_time);
        btn_choosetime.setOnClickListener(this);
        btn_confirm = (Button) findViewById(R.id.stu_FINALLY);
        btn_confirm.setOnClickListener(this);
        IDS = (EditText) findViewById(R.id.stu_ids);
    }

    @Override
    public void onClick(View v)
    {
        if (v.getId() == R.id.stu_time)
        {
            final Calendar c = Calendar.getInstance();
            DatePickerDialog dialog = new DatePickerDialog(stu_seacher.this, new DatePickerDialog.OnDateSetListener()
            {
                @Override
        ...
        else if (v.getId() == R.id.stu_FINALLY)
        {
            if (flag == 1)
            {    //选了日期
                if (IDS.getText().toString().trim().equals(""))
                {
                    ...
                    SQLiteDatabase db;
                    MyHelper ggg = new MyHelper(stu_seacher.this);
                    ...
                    if (cursor.getCount() != 0)
                    {
                        ...
                        if (TIMES.substring(0, TIMES.length()).equals
                                (name_time.substring(0, TIMES.length())))
                        {
                    ...
                    String TIMES = btn_choosetime.getText().toString().trim();   //日期
                    SQLiteDatabase db;
                    MyHelper ggg = new MyHelper(stu_seacher.this);
                    db = ggg.getWritableDatabase();
                    StringBuffer sum = new StringBuffer();
