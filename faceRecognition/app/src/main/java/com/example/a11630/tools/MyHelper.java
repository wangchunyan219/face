...
public class MyHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        String stu_table = "create table name_id (id text primary key ,name text)"; //name----id_
        String stu_table1 = "create table time_id(id text ,time text)"; //time----id
        String  stu_table2 = "create table password_id(id text primary key,password text)";//password----id
  ...
  public  void Insert(SQLiteDatabase db, String table, String name, String id) //db,table,name,id
  {
    ...
    public  void Insert_three(SQLiteDatabase db, String table, String password, String id)  //db,table,password,id
    {
        ContentValues cValue = new ContentValues();
        cValue.put("id", id);
        cValue.put("password", password);
        db.insert(table, null, cValue);
        System.out.println("插入password成功："+ id +"   "+ password);
    }
